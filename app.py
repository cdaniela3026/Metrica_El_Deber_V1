...
try:
    ...
    # --- Entrada competencia para comparativa
    yt_comp_q = st.session_state.get("ytc_q", os.getenv("VIDEO_ID_COMP", "")) if 'st' in globals() else os.getenv("VIDEO_ID_COMP", "")
    try:
        yt_comp = fetch_youtube_concurrent(yt_comp_q) if 'fetch_youtube_concurrent' in globals() else {"value": 0, "status":"warn", "note":"función faltante"}
    except Exception:
        yt_comp = {"value": 0, "status":"warn", "note":"error consultando competencia"}

    
    # Layout: izquierda (donut EL DEBER vs COMPETENCIA) / derecha (logos + barras)
    left_col, right_col = st.columns([2,3])

    
    with left_col:
        #st.markdown("**Comparativa en vivo — YouTube EL DEBER vs COMPETENCIA**")
        _yt_val_hist = int(st.session_state.get("yt_hist", [])[-1].get("vistas", 0)) if st.session_state.get("yt_hist") else 0
        _ytc_val_hist = int(st.session_state.get("ytc_hist", [])[-1].get("vistas", 0)) if st.session_state.get("ytc_hist") else 0
        _yt_val = _yt_val_hist or int(yt.get("value", 0) or 0)
        _ytc_val = _ytc_val_hist or int(yt_comp.get("value", 0) or 0)
        _vals_sum = _yt_val + _ytc_val
        if _vals_sum <= 0:
        
            # Fallback para visualizar el donut aún sin datos reales
            df_yt_donut = pd.DataFrame([
                {"name": "YouTube Vivo EL DEBER", "value": 1},
                {"name": "YouTube Vivo COMPETENCIA", "value": 1},
            ])
            _no_data_donut = True
        else:
            df_yt_donut = pd.DataFrame([
                {"name": "YouTube Vivo EL DEBER", "value": _yt_val},
                {"name": "YouTube Vivo COMPETENCIA", "value": _ytc_val},
            ])
            _no_data_donut = False
        fig_yt_donut = px.pie(df_yt_donut, names="name", values="value", hole=0.55,
                              title="Comparativa en vivo — YouTube EL DEBER vs COMPETENCIA")
        fig_yt_donut.update_traces(textposition="inside", textinfo="label+value",
                                   marker=dict(colors=["#16a34a","#ef4444"],
                                   ),
                                   domain=dict(x=[0.0, 0.68], y=[0.0, 1.0]))
        fig_yt_donut.update_layout(height=300, margin=dict(l=10, r=30, t=30, b=10), showlegend=True,
                                   legend=dict(orientation="v", y=0.5, yanchor="middle", x=0.98, xanchor="right"))
        st.plotly_chart(fig_yt_donut, use_container_width=True)
        if _no_data_donut:
            st.caption("Sin datos de YouTube y/o competencia para comparar (mostrando donut neutral).")
    ...
    with right_col:
        ...
        df_live = pd.DataFrame([
            {"platform":"YouTube", "vistas": yt.get("value",0)},
            {"platform":"TikTok",  "vistas": tt.get("value",0)},
            {"platform":"Facebook","vistas": fb.get("value",0)},
            {"platform":"X", "vistas": 0},
        ])
        fig_top = px.bar(
            df_live, x="platform", y="vistas", text="vistas",
        ...
        _ymax = max(1, int(df_live["vistas"].max() or 0))
...
with tab_tt:
    ...
    if tt_auto and st.session_state.get("tt_run") and tt_user:
        st_autorefresh(interval=30000, key="tt_live_auto")
...
def render_youtube_tab(tab_container, DEFAULT_VIDEO_ID=""):
    with tab_container:
        #st.subheader("YouTube EL DEBER en vivo")
        st.markdown(
    "<span style='font-size:1.8rem; font-weight:900; color:#16a34a;'>YouTube EL DEBER en vivo</span>",
    unsafe_allow_html=True
)
        ...
        if st.session_state.get("yt_run") and video_id:
            try:
                ...
                if not ok:
                    ...
                    yt_snap = {
                        "ts": pd.Timestamp.utcnow(),
                        "vistas": concurrent or view_count,
                        "likes": like_count,
        ...
        if st.session_state["yt_hist"]:
            ...
            fig_y = px.line(
                df_y, x="ts", y=["vistas", "likes", "comments"],
                title="Evolución en vivo — YouTube",
                markers=True,
            )
            fig_y.update_layout(margin=dict(l=10, r=10, t=40, b=0), height=320, legend_title_text="")
            fig_y.update_traces(line=dict(color="#16a34a"))
            st.plotly_chart(fig_y, use_container_width=True)
            row = df_y.iloc[-1]
            df_y_last = pd.DataFrame({
                "metric": ["vistas", "likes", "comments"],
                "value":  [int(row.get("vistas", 0)), int(row.get("likes", 0)), int(row.get("comments", 0))]
            })
            cb1, cb2 = st.columns(2)
            fig_bar = px.bar(df_y_last, x="metric", y="value", text="value", title="YouTube — snapshot (barras)")
            fig_bar.update_traces(textposition="outside", cliponaxis=False, marker_color="#16a34a")
            fig_bar.update_layout(margin=dict(l=10, r=10, t=40, b=0), height=320, showlegend=False)
            ...
            fig_pie.update_traces(textposition="inside", texttemplate='%{percent:.1%}', marker=dict(colors=["#bbf7d0","#86efac","#22c55e"]))
            fig_pie.update_layout(margin=dict(l=10, r=10, t=40, b=0), height=320)
            cb2.plotly_chart(fig_pie, use_container_width=True)


def render_youtube_competencia(tab_container, DEFAULT_VIDEO_ID_COMP=""):
    with tab_container:
        st.markdown(
    "<span style='font-size:1.8rem; font-weight:900; color:#FF0000;'>YouTube COMPETENCIA en vivo</span>",
    unsafe_allow_html=True
)

        # Estado propio para competencia
        if "ytc_q" not in st.session_state:
            st.session_state["ytc_q"] = ""
        if "ytc_run" not in st.session_state:
            st.session_state["ytc_run"] = False
        st.session_state.setdefault("ytc_hist", [])

        q = st.text_input(
            "Pega la URL o ID de un video en vivo (también acepta youtu.be).",
            value=DEFAULT_VIDEO_ID_COMP,
            placeholder="https://www.youtube.com/watch?v=VIDEO_ID o VIDEO_ID",
            key="ytc_input_v2",
        )

        colA, colB, colC = st.columns([1, 1, 2])
        run_click = colA.button("Consultar", type="primary", key="ytc_btn_v2")
        yt_auto   = colB.toggle("Auto-actualizar 30s", value=True, key="ytc_auto_v2")
        yt_debug  = colC.toggle("Debug", value=False, key="ytc_dbg_v2")

        if run_click and q:
            st.session_state["ytc_q"] = q
            st.session_state["ytc_run"] = True
            st.session_state["ytc_hist"] = []

        raw_query = st.session_state.get("ytc_q", "")
        video_id = yt_normalize_id(raw_query)
        st.caption(f"Backend: {LOCAL_API} — Consultando video_id (competencia): {video_id or '—'}")

        if yt_auto and st.session_state.get("ytc_run") and video_id:
            st_autorefresh(interval=30000, key="ytc_live_auto_v2")

        title_with_icon("Métricas en vivo", METRIC_ICON, size=26)
        yt_snap = None
        dbg_raw = None

        if st.session_state.get("ytc_run") and video_id:
            try:
                ok, data, status = api_get_debug("/live-data", params={"video": video_id})
                dbg_raw = {"ok": ok, "status": status, "payload": data}
                items = []
                ...
                st.error(f"No se pudo obtener datos de YouTube competencia: {e}")

        else:
            st.info("Pega una URL/ID de un video en vivo y presiona **Consultar**.")

        # Historial y gráficas (línea + snapshot barras/donut) en ROJO
        if yt_snap is not None:
            st.session_state["ytc_hist"] = (st.session_state["ytc_hist"] + [yt_snap])[-200:]

        if st.session_state["ytc_hist"]:
            df_y = pd.DataFrame(st.session_state["ytc_hist"])

            fig_y = px.line(
                df_y, x="ts", y=["vistas", "likes", "comments"],
                title="Evolución en vivo — YouTube competencia",
                markers=True,
            )
            fig_y.update_layout(margin=dict(l=10, r=10, t=40, b=0), height=320, legend_title_text="")
            fig_y.update_traces(line=dict(color="#ef4444"))
            st.plotly_chart(fig_y, use_container_width=True)
            ...
            fig_bar = px.bar(df_y_last, x="metric", y="value", text="value", title="YouTube competencia — snapshot (barras)")
            fig_bar.update_traces(textposition="outside", cliponaxis=False, marker_color="#ef4444")
            fig_bar.update_layout(margin=dict(l=10, r=10, t=40, b=0), height=320, showlegend=False)
            cb1.plotly_chart(fig_bar, use_container_width=True)

            fig_pie = px.pie(df_y_last, names="metric", values="value", hole=0.55, title="YouTube competencia — snapshot (donut)")
...
render_youtube_competencia(tab_yt, os.getenv('VIDEO_ID_COMP', ''))
    ...
    if st.session_state["fb_hist"]:
        ...
        if y_cols_f:
            fig_f = px.line(
                ...
                color_discrete_map={
                    "likes": PLATFORM_COLORS["Facebook"],
                    "shares": "#16a34a",
                },
                markers=True,
            )
